use crate::{app::{App, SelectedComponent}, components::dialog::DialogComponent};


pub fn open_help_menu(app: &mut App) {
    // Tasks that are universal
    let mut actions: Vec<(String, Box<dyn Fn(&mut App)>)> = vec![
        (
    ...
    app.dialog_stack.push_front(DialogComponent::new(String::from("Help Menu"), actions));
}
