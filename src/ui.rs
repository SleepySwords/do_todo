...
use crate::{app::App, task::Priority};
use crate::task::Task;
...
pub fn ui<B: Backend>(app: &mut App, f: &mut Frame<B>) {
    ...
    let tasks: Vec<ListItem> = app.tasks.iter().enumerate().map(|(i, task)| {
        let progess = Span::styled(if task.progress { "[-] " } else { "[ ] " }, Style::default().fg(task.priority.get_colour()));
        let content = Span::styled(task.content.as_str(), Style::default().fg(if i == app.selected_index { theme.selected_colour } else { Color::White }));
        let content = Spans::from(vec![progess, content]);

        ListItem::new(content)
    }).collect();

    let completed_tasks: Vec<ListItem> = app.completed_tasks.iter().map(|task| {
...

    {
        ...
        let current = List::new(tasks)
            .block(Block::default().title("Current List").borders(Borders::ALL).border_type(BorderType::Rounded).border_style(Style::default().fg(theme.default_colour)))
            .style(Style::default().fg(Color::White));
            // .highlight_style(Style::default().add_modifier(Modifier::BOLD).fg(theme.selected_colour));
        ...
        let recently_competed = List::new(completed_tasks)
            .block(Block::default().title("Completed").borders(Borders::ALL).border_type(BorderType::Rounded).border_style(Style::default().fg(theme.default_colour)))
            .style(Style::default().fg(Color::White));
    ...
    loop {
        ...
        if let Event::Key(key) = event::read()? {
            ...
            match key.code {
                ...
                KeyCode::Char('h') => {
                    if app.tasks.len() == 0 { continue; }
                    app.tasks[app.selected_index].priority = app.tasks[app.selected_index].priority.get_next();
                }
                KeyCode::Char('p') => {
                    if app.tasks.len() == 0 { continue; }
                    app.tasks[app.selected_index].progress = !app.tasks[app.selected_index].progress;
                }
                KeyCode::Char('c') => {
                    let local = Local::now();
